...
`radix-nova`, base color `neutral`, CSS-variable theming, `tsx: true` — the frontend is
TypeScript, so `shadcn add` generates `.tsx`).
...
- `UserAvatar.tsx` — `UserAvatar({ username, size, className, fallbackClassName })`, the
  avatar placeholder: shadcn `Avatar` + `AvatarFallback` showing the username's initial on
  a colour derived from the username (`lib/avatar-color.ts`, Pulse tints with per-tint text
  ...
  the stock `data-[size=lg]:size-10` / `data-[size=sm]:size-6` in `ui/avatar.tsx`.
- `UserName.tsx` — `UserName({ username, displayName, stacked = false, className, nameClassName,
  ...
  (2026-09-24).**
- `BrandMark.tsx` — `BrandMark({ className, iconClassName })`, the logo: lucide `Feather` in a
  `bg-primary` rounded square, decorative (`aria-hidden`). No shadcn primitive. Used by
  `SideNav` and `AuthLayout`.
- `AuthLayout.tsx` — `AuthLayout({ title, className, children })`, the centred frame for
...
The Pulse ports below (`components/layout/`, `feed/Composer.tsx`) were migrated at Alejandro's
...
- `layout/AppShell.tsx` — the layout route for every gated page: three columns (`SideNav`,
...
- `layout/SideNav.tsx` — left rail: brand, nav from `nav-items.ts`, "New post", user chip,
  Sign out. Uses `Button` (+ `UserAvatar`, `BrandMark`, `ComingSoon`).
- `layout/MobileNav.tsx` — bottom bar below `lg`, icons only, same nav config. No shadcn
  primitive (react-router `NavLink` + `ComingSoon`).
- `layout/RightRail.tsx` — right rail (`xl`): the live search typeahead (user-search feature —
  ...
  `FollowButton`).
- `layout/ComingSoon.tsx` — `ComingSoon({ children, side })`, the disabled-feature wrapper:
...
- `layout/PageHeader.tsx` — `PageHeader({ title, subtitle, leading, trailing, children,
...
- `feed/PostCard.tsx` — `PostCard({ post, variant = 'card' | 'detail', onDeleted, className })`:
...
- `feed/CommentComposer.tsx` — `CommentComposer({ postId, textareaRef })`, the reply box on the
  detail page; same rules as `Composer` ("Reply"). Uses `Textarea`, `Button`, `Spinner`
  (+ `UserAvatar`, `CharacterCounter`).
- `feed/CommentItem.tsx` — `CommentItem({ comment, postId, onDeleted })`: avatar, `UserName`,
  ...
  `UserName`).
- `feed/InfiniteListFooter.tsx` — `InfiniteListFooter({ query, endMessage, loadMoreLabel,
...
- `feed/CharacterCounter.tsx` — `CharacterCounter({ id, length, remaining })`, the mono
...
- `feed/PostListSkeleton.tsx` — `PostListSkeleton({ label, count })`, the loading placeholder for
...
- `FollowButton.tsx` — `FollowButton({ username, isFollowing, followsYou = false, className })`,
...
- `FollowListDialog.tsx` — `FollowListDialog({ username, tab, onTabChange, onClose, isOwnProfile
...
Explore (`pages/Explore.tsx`, user-search feature, listed in the feature plan Alejandro framed) is
...
Followers" row is plain buttons inside `Profile.tsx`, not a component.
...
- `hooks/use-mobile.ts` — `useIsMobile()`, matchMedia 768px breakpoint (stock shadcn
...
- `lib/utils.ts` — `cn()`, the stock shadcn `clsx` + `tailwind-merge` helper.
- `hooks/use-debounced-value.ts` — `useDebouncedValue(value, delayMs)`, the value once it has
  stopped changing for `delayMs` (the search typeahead and Explore's URL sync).
- `lib/text.ts` — composer body helpers (`POST_MAX_LENGTH`, `countCodePoints`, `measureBody`,
  `limitAnnouncement`, `isSubmitShortcut`); `lib/format.ts` — `formatCount`, `formatRelativeShort`, `formatFullDate`
  (date-fns); `lib/composer-focus.ts` — focusing the Home composer from "New post".
- `lib/navigation-history.ts` — `useCanGoBackInApp()`, whether `navigate(-1)` stays inside the
  app (used by PostDetail's Back); fed by `app/NavigationDepthTracker.tsx`, a provider at the
