...
const explorePath = (query: string) => `/explore?q=${encodeURIComponent(query)}`
// Same accessible-name convention as the other user links (T4): "Display Name @username", or just
// "@username" without a display name.
const userLabel = ({ username, displayName }: Pick<FollowUser, 'username' | 'displayName'>) =>
...
//
// Hand-rolled on purpose instead of the shadcn primitives in `components/ui/`: `command.tsx` (cmdk)
// filters and ranks items itself and owns the input, `combobox.tsx` (Base UI) is a value picker
// (choosing an item sets the input's value) with its own filtering, and `popover.tsx` (Radix)
...
function ProfileCard({ username, authDisplayName }: ProfileCardProps) {
  const titleId = useId()
...
function SuggestionRow({ suggestion }: { suggestion: FollowUser }) {
