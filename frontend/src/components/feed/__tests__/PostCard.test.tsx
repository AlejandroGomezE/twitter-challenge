...
import type { Post, PostAuthor } from '@/lib/api/types'
import { renderWithProviders } from '@/test/render'
import { apiUrl, server } from '@/test/server'
import { PostCard } from '../PostCard'

// A post as the API sends it (the author's `displayName` left out unless a test sets it).
type PostFixture = Omit<Post, 'author'> & {
  author: Omit<PostAuthor, 'displayName'> & Partial<Pick<PostAuthor, 'displayName'>>
}

const post = (overrides: Partial<PostFixture> = {}): PostFixture => ({
  id: 'p1',
  body: 'hello flock',
...
async function renderCard(overrides?: Partial<PostFixture>) {
  server.use(
