...
function ThingList() {
  const query = useInfiniteQuery({
    queryKey: ['things'],
    queryFn: ({ pageParam }) =>
      apiClient.get<Page<string>>(pageParam ? `/things?cursor=${pageParam}` : '/things'),
  ...
  return (
    <>
      <ul>
        {/* Not pending, and this list never fails its first page: the data is there. */}
...
function mockThings({ gate }: { gate?: Promise<void> } = {}) {
...
// The part of an IntersectionObserver callback the footer reads.
...
describe('InfiniteListFooter', () => {
  ...
  it('still loads the next page when the sentinel fires during a refetch of the list', async () => {
    ...
    const live = new Set<ControlledObserver>()
    let inView = false
