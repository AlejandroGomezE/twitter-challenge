...
export function CommentItem({ comment, postId, onDeleted }) {
  const { user } = useAuth();
  const username = comment.author.username;
  const profilePath = `/u/${encodeURIComponent(username)}`;
  const isOwn = Boolean(user?.username) && user.username.toLowerCase() === username.toLowerCase();
...
function CommentMenu({ comment, postId, onDeleted }) {
