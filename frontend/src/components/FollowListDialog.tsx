...
type FollowListQuery = UseInfiniteQueryResult<InfiniteData<Page<FollowUser>>>;

const TABS: { value: FollowListTab; label: string }[] = [
  { value: 'following', label: 'Following' },
  { value: 'followers', label: 'Followers' },
];

const isFollowListTab = (value: string): value is FollowListTab =>
  TABS.some((tab) => tab.value === value);

export interface FollowListDialogProps {
  username: string;
  tab: FollowListTab | null;
...
export function FollowListDialog({
  ...
  const [shownTab, setShownTab] = useState<FollowListTab>(tab ?? 'following');
  ...
  const queries: Record<FollowListTab, FollowListQuery> = { following, followers };
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      ...
      >
        ...
        <Tabs
          value={shownTab}
          onValueChange={(value) => isFollowListTab(value) && onTabChange(value)}
...
function FollowList({ kind, query, username, isOwnProfile, onNavigate }: FollowListProps) {
  const { user } = useAuth();
...
function emptyMessage(kind: FollowListTab, username: string, isOwnProfile: boolean) {
  if (kind === 'following') {
    return isOwnProfile
...
function FollowListRow({ item, isMe, onNavigate }: FollowListRowProps) {
  return (
...
function FollowListSkeleton({ count = 4 }: { count?: number }) {
  return (
    <div aria-busy="true">
