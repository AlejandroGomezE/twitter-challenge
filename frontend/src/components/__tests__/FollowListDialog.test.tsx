...
import { FollowListDialog, type FollowListTab } from '@/components/FollowListDialog'
import type { FollowUser } from '@/lib/api/types'
import { renderWithProviders } from '@/test/render'
import { apiUrl, server } from '@/test/server'

// A list row as the API sends it (`displayName` left out unless a test sets it).
type FollowRow = Partial<FollowUser> & { username: string }
type FollowLists = Partial<Record<FollowListTab, FollowRow[][]>>
interface FollowListsState {
  requests: string[]
  status: Partial<Record<FollowListTab, number>>
  hold: Promise<void> | null
}

const followUser = (username: string, overrides: Partial<FollowUser> = {}): FollowRow => ({
  username,
  bio: `bio of ${username}`,
  isFollowing: false,
  followsYou: false,
...
function mockFollowLists(lists: FollowLists) {
  const state: FollowListsState = { requests: [], status: {}, hold: null }
...
interface HarnessProps {
  username?: string
  initialTab?: FollowListTab | null
  isOwnProfile?: boolean
}

function Harness({ username = 'grace', initialTab = null, isOwnProfile = false }: HarnessProps) {
  const [tab, setTab] = useState<FollowListTab | null>(initialTab)
  return (
    <>
...
describe('FollowListDialog', () => {
  ...
  describe('freshness', () => {
    ...
    function holdResponses(state: FollowListsState) {
      let release: () => void
