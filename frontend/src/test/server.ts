...
// Matches `test.env.VITE_API_URL` in vite.config.ts — build handler URLs with `apiUrl()`.
...
// after each test (src/test/setup.ts). Unhandled requests fail the test.
export const handlers = [
  ...
  http.get<UsernameParams>(apiUrl('/users/:username/posts'), ({ params }) =>
    params.username.toLowerCase() === 'ada'
  ...
  ...['followers', 'following'].map((list) =>
    http.get<UsernameParams>(apiUrl(`/users/:username/${list}`), ({ params }) =>
      params.username.toLowerCase() === 'ada'
        ? HttpResponse.json({ items: [], nextCursor: null })
        : HttpResponse.json({ message: 'User not found' }, { status: 404 }),
    ),
