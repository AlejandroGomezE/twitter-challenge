...
const getEventSource = (): typeof EventSource | null =>
...
function addToMap<K, H>(map: Map<K, Set<H>>, key: K, handler: H) {
...
// A stored event handler: gets the parsed body of its own event's messages.
...
export function RealtimeProvider({ children }: RealtimeProviderProps) {
  ...
  const [streamStatus, setStatus] = useState<RealtimeStatus>(REALTIME_STATUS.connecting)
  const status =
  ...
  const eventHandlersRef = useRef(new Map<string, Set<AnyEventHandler>>())
  ...
  const bind = useCallback((eventName: string) => {
  ...
  const subscribe = useCallback<RealtimeApi['subscribe']>(
    (eventName, handler) => {
      // Stored untyped by event name: `bind` only ever passes a handler its own event's payload.
      const unsubscribe = addToMap(
  ...
  useEffect(() => {
    ...
    const connect = () => {
      timer = null
      // `current` is this connection's source (the onerror below always refers to it).
      const current = new EventSourceImpl(`${API_URL}/events`, { withCredentials: true })
      source = current
      ...
      source.onerror = () => {
        if (current.readyState !== (EventSourceImpl.CLOSED ?? CLOSED)) {
  ...
  const api = useMemo<RealtimeApi>(() => ({ subscribe, subscribeReconnect }), [subscribe, subscribeReconnect])
