...
// Feed fixtures only carry post ids.
type FakePage = Page<Pick<Post, 'id'>>
type FakeFeed = InfiniteData<FakePage>

// A feed page holding posts with the given ids.
const page = (ids: string[], nextCursor: string | null = null): FakePage => ({ items: ids.map((id) => ({ id })), nextCursor })
// Infinite-query data with one page per ids array.
const feedData = (...pages: string[][]): FakeFeed => ({ pages: pages.map((ids) => page(ids)), pageParams: pages.map(() => null) })

// The same infinite-query options Home's feeds use (hooks/use-posts.ts).
const feedOptions = (tab: FeedTab) => ({
  queryKey: feedQueryKey(tab),
  queryFn: ({ pageParam }: { pageParam: string | null }) => (tab === FOR_YOU ? fetchForYouFeed : fetchFeed)(pageParam),
  initialPageParam: null as string | null,
  getNextPageParam: (lastPage: Page<Post>) => lastPage.nextCursor ?? undefined,
})
...
const gatedFeed = (g: ReturnType<typeof gate>, body: FakePage) =>
  server.use(
...
describe('syncNewPostsWithFeeds', () => {
  ...
  it('a "load more" (fetchNextPage) does not clear pending ids', async () => {
    ...
    expect(queryClient.getQueryData<FakeFeed>(postKeys.feed())?.pages).toHaveLength(2)
  ...
  it('a manual cache write (e.g. an optimistic prepend) does not count as a refetch', () => {
    ...
    queryClient.setQueryData<FakeFeed>(
      postKeys.feed(),
      (data) =>
        data && {
          ...data,
