...
import type { PostListData } from '@/lib/api/post-cache'
...
export type FeedTab = (typeof NEW_POSTS_TABS)[keyof typeof NEW_POSTS_TABS]

// The feed query behind each tab.
export const feedQueryKey = (tab: FeedTab) =>
...
export interface NewPostsStore {
  getIds(tab: FeedTab): readonly string[]
  subscribe(listener: () => void): () => void
  add(id: string, following: boolean): void
  remove(id: string): void
  removeFromTab(tab: FeedTab, ids: readonly string[]): void
  clear(tab: FeedTab): void
}

type NewPostsState = Record<FeedTab, readonly string[]>

export function createNewPostsStore(): NewPostsStore {
  // `Object.fromEntries` can't know the keys are exactly TAB_IDS.
  let state = Object.fromEntries(TAB_IDS.map((tab) => [tab, EMPTY])) as NewPostsState
  const listeners = new Set<() => void>()

  const setTab = (tab: FeedTab, ids: readonly string[]) => {
    if (ids === state[tab]) return
    state = { ...state, [tab]: ids }
    for (const listener of [...listeners]) listener()
  }

  const without = (ids: readonly string[], drop: Set<string>) => {
...
export const NewPostsContext = createContext<NewPostsStore | null>(null)

const tabOfQuery = (query: Query) =>
  TAB_IDS.find((tab) => matchQuery({ queryKey: feedQueryKey(tab), exact: true }, query))

const idsInFeed = (data?: PostListData) => data?.pages?.flatMap((page) => page.items.map((post) => post.id)) ?? []
...
export function syncNewPostsWithFeeds(queryClient: QueryClient, store: NewPostsStore) {
  // tab -> ids pending when its current first-page fetch started.
  const inFlight = new Map<FeedTab, readonly string[]>()
  ...
  return queryClient.getQueryCache().subscribe((event) => {
    ...
    switch (action.type) {
      case 'fetch':
        ...
        break
      case 'success': {
        // A feed query's data is always a post list.
        const ids = idsInFeed(query.state.data as PostListData | undefined)
        const pending = inFlight.get(tab)
        if (!action.manual && pending) {
          ids.push(...pending)
          inFlight.delete(tab)
        }
