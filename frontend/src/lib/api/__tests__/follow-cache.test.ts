...
type Row = Pick<FollowUser, 'username' | 'bio' | 'isFollowing'>
type Rows = InfiniteData<Page<Row>>

const row = (username: string, isFollowing: boolean): Row => ({ username, bio: null, isFollowing })
const infinite = (...pages: Row[][]): Rows => ({
...
describe('setFollowingInLists', () => {
  it("updates only the target's rows across followers, following and suggestions", () => {
    ...
    const followers = queryClient.getQueryData<Rows>(followKeys.followers('bob'))
    expect(followers?.pages[0].items).toEqual([row('carol', false), row('Ada', true)])
    expect(followers?.pages[1].items).toEqual([row('dave', false)])
    expect(queryClient.getQueryData<Rows>(followKeys.following('carol'))?.pages[0].items).toEqual([
      row('ada', true),
    ])
    expect(queryClient.getQueryData<Pick<Page<Row>, 'items'>>(followKeys.suggestions())?.items).toEqual([
      row('eve', false),
      row('ADA', true),
    ])
  })
  ...
  it('keeps pages and lists without the target (or already in that state) by reference', () => {
    ...
    const updated = queryClient.getQueryData<Rows>(followKeys.followers('zed'))
  ...
  it("updates the target's search result rows, in both the typeahead and Explore shapes", () => {
    ...
    const explore = queryClient.getQueryData<Rows>(searchKeys.userResults('a', 'all'))
...
describe('bumpProfileFollowingCount', () => {
  it("adds the delta to a cached profile's followingCount (case-insensitive key)", () => {
    ...
    expect(queryClient.getQueryData<Profile>(profileQueryKey('bob'))?.followingCount).toBe(3)

    bumpProfileFollowingCount(queryClient, 'bob', -1)
    expect(queryClient.getQueryData<Profile>(profileQueryKey('bob'))?.followingCount).toBe(2)
  })
  ...
  it('never goes below 0', () => {
    ...
    expect(queryClient.getQueryData<Profile>(profileQueryKey('bob'))?.followingCount).toBe(0)
  })
