import { QueryObserver, type QueryClient, type QueryFunction } from '@tanstack/react-query'
...
function gatedQueryFn(answers: string[]) {
  const gates: (() => void)[] = []
  const queryFn = () =>
  ...
  return { queryFn, calls: () => gates.length, open: (i: number) => gates[i]() }
}
...
const observe = (queryClient: QueryClient, queryFn: QueryFunction<string>) =>
