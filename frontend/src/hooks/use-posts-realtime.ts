...
import type { RealtimeEventMap } from '@/lib/api/types';
...
// `data` is checked field by field (a malformed message is ignored).
function applyCounts(
  queryClient: QueryClient,
  data?: Partial<RealtimeEventMap['post.counts']> | null,
) {
...
async function applyDeleted(
  queryClient: QueryClient,
  data?: Partial<RealtimeEventMap['post.deleted']> | null,
) {
  const id = data?.id;
