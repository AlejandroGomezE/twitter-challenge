...
export function useFollowers(username: string, options?: FollowListOptions) {
...
export function useFollowing(username: string, options?: FollowListOptions) {
  return useFollowList(followKeys.following(username), fetchFollowing, username, options);
}

function useFollowList(
  queryKey: QueryKey,
  fetchPage: (username: string, cursor?: string | null) => Promise<Page<FollowUser>>,
  username: string,
  { enabled = true, alwaysFresh = false }: FollowListOptions = {},
) {
...
// use-posts.ts: `confirmed` is the last state the server is known to hold — the cached state when
...
interface FollowBurst {
  ...
  confirmedSeq: number;
  confirmed: CachedFollowState | null;
  shown: boolean | null;
  ownCountKnown: boolean;
  ownCountFetched: boolean;
  succeeded: boolean;
  // The signed-in user's username (null when unknown or following themselves).
  me: string | null;
}

const followBursts = new WeakMap<QueryClient, Map<string, FollowBurst>>();

function getFollowBursts(queryClient: QueryClient) {
...
const isLiveBurst = (queryClient: QueryClient, key: string, burst: FollowBurst) =>
  followBursts.get(queryClient)?.get(key) === burst;
...
export interface FollowCacheUser {
  username: string;
  isFollowing?: boolean;
...
export function withPendingFollow<U extends FollowCacheUser>(queryClient: QueryClient, user: U): U {
...
// cache.
export function resetFollowBursts(queryClient: QueryClient) {
...
export function useToggleFollow() {
  ...
  return useMutation({
    mutationFn: ({ username, following }: ToggleFollowVariables) => setFollowing(username, following),
    onMutate: async ({ username, following }: ToggleFollowVariables) => {
    ...
    onSettled: (result, error, { username }, context) => {
      ...
      if (!error) {
        ...
        if (result && context.seq > burst.confirmedSeq) {
