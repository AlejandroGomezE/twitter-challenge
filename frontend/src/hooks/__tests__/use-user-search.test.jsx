...
function captureSearches(respond = () => page([])) {
  const seen = []
  server.use(
...
const usernames = (items) => items.map((item) => item.username)
...
describe('follow state in search results', () => {
  ...
  it('flips the row in both result shapes optimistically and keeps the confirmed state', async () => {
    ...
    expect(
      queryClient.getQueryData(searchKeys.userResults('bo', 'typeahead')).items[0].isFollowing,
    ).toBe(true)
  })
