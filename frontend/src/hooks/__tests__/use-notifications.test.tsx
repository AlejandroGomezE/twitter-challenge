...
function renderWithClient<T>(useHooks: () => T, { realtime = false }: { realtime?: boolean } = {}) {
...
// The query's options, which include its observer's (`refetchInterval`, `refetchOnWindowFocus`).
const unreadCountOptions = (queryClient: QueryClient): Partial<QueryObserverOptions> =>
...
function mockUnreadCount(...counts: number[]) {
  const calls = { count: 0 }
  server.use(
...
describe('useNotifications', () => {
  ...
  it('caches the pages under the notifications list key', async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
    expect(queryClient.getQueryData<InfiniteData<Page<Notification>>>(notificationKeys.list())?.pages).toHaveLength(1)
  })
})
