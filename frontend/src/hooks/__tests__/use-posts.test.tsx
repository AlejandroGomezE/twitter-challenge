...
import type { LikeState, Page, Post, PostAuthor, Profile } from '@/lib/api/types'
...
// Fixture authors only carry a username.
type PostOverrides = Partial<Omit<Post, 'author'>> & { author?: Pick<PostAuthor, 'username'> }

const post = (id: string, overrides: PostOverrides = {}) => ({
  id,
  body: `post ${id}`,
...
function mockFeed(initial: TestPost[], { refetched = initial, refetchGate }: MockFeedOptions = {}) {
  let calls = 0
  server.use(
...
const likeHandler = (method: 'put' | 'delete', g: Gate, result: LikeState | null) =>
...
// The Following feed's cached posts (throws when the feed isn't cached).
const feedItems = (queryClient: QueryClient) => {
  const data = queryClient.getQueryData<InfiniteData<Page<TestPost>>>(postKeys.feed())
  if (!data) throw new Error('The feed is not cached')
  return data.pages.flatMap((page) => page.items)
}

const likeState = (queryClient: QueryClient, id = 'p1') => {
...
describe('useToggleLike', () => {
  ...
  interface DoubleClickOptions {
    putResult: LikeState | null
    deleteResult: LikeState | null
    order: ('put' | 'delete')[]
  }
...
describe('useCreatePost / useDeletePost', () => {
  it('keeps a created post when a stale feed refetch lands afterwards', async () => {
    ...
    expect(queryClient.getQueryData<Profile>(profileQueryKey('ada'))?.postCount).toBe(2)
  })
  ...
  it('keeps a deleted post out when a stale feed refetch lands afterwards', async () => {
    ...
    expect(queryClient.getQueryData<Profile>(profileQueryKey('ada'))?.postCount).toBe(1)
  })
})

describe('useForYouFeed', () => {
  const forYouItems = (queryClient: QueryClient) => {
    const data = queryClient.getQueryData<InfiniteData<Page<TestPost>>>(postKeys.forYou())
    if (!data) throw new Error('The For you feed is not cached')
    return data.pages.flatMap((page) => page.items)
  }
...
describe('like bursts across sign-out', () => {
  const seedFeed = (queryClient: QueryClient, items: TestPost[]) =>
