...
function mockComments(page, { refetched = page, refetchGate } = {}) {
  let calls = 0
  server.use(
...
const commentIds = (queryClient) =>
  queryClient
    .getQueryData(postKeys.comments('p1'))
    .pages.flatMap((page) => page.items.map((item) => item.id))

// [detail, feed] commentCount of p1.
const commentCounts = (queryClient) => [
  queryClient.getQueryData(postKeys.detail('p1')).commentCount,
  queryClient.getQueryData(postKeys.feed()).pages[0].items[0].commentCount,
]
