...
const profile = (username, overrides = {}) => ({
  username,
  bio: null,
...
const infinite = (items) => ({ pages: [page(items)], pageParams: [null] })
...
const followHandler = (method, g, result) =>
...
async function renderToggle(extraHooks = () => ({})) {
...
const snapshot = (queryClient) => {
  const bob = queryClient.getQueryData(profileQueryKey('bob'))
  const listRow = queryClient
    .getQueryData(followKeys.followers('carol'))
    .pages[0].items.find((item) => item.username === 'bob')
  return {
    isFollowing: bob.isFollowing,
    followerCount: bob.followerCount,
    myFollowingCount: queryClient.getQueryData(profileQueryKey('ada')).followingCount,
    listRow: listRow.isFollowing,
    suggestion: queryClient.getQueryData(followKeys.suggestions()).items[0].isFollowing,
  }
}
...
const settled = (queryClient) => waitFor(() => expect(queryClient.isMutating()).toBe(0))
...
describe('useToggleFollow', () => {
  it('flips every cached copy optimistically, then reconciles and invalidates', async () => {
    ...
    expect(
      queryClient.getQueryData(followKeys.followers('carol')).pages[0].items[0],
    ).toEqual(row('dan'))
    ...
    const invalidated = (key) => queryClient.getQueryState(key).isInvalidated
  ...
  describe('the caller’s own profile fetched mid-burst', () => {
    ...
    const myCount = (queryClient) => queryClient.getQueryData(profileQueryKey('ada')).followingCount
    const refetchMe = (queryClient) =>
