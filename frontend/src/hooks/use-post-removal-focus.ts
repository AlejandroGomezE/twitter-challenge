...
// `onDeleted`.
// What this needs from a rendered post.
...
export function usePostRemovalFocus(items?: readonly RenderedPost[]) {
  ...
  const renderedRef = useRef<readonly RenderedPost[] | undefined>(undefined);
  ...
  return function handleDeleted(postId: string) {
...
function focusPostOrMain(postId: string | null) {
  const card = [...document.querySelectorAll<HTMLElement>('article[data-post-id]')].find(
    (article) => article.dataset.postId === postId,
  );
  const target = card?.querySelector<HTMLElement>('[data-post-link]') ?? document.getElementById('main-content');
  target?.focus();
}
