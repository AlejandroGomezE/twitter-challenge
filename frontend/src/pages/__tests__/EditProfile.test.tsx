...
import type { MyProfile, Profile, ProfilePatch } from '@/lib/api/types'
...
// What the fake API stores (and serves from `GET /users/:username`) per user. A saved PATCH
// response may leave `displayName` / `bio` out.
type StoredProfile = Pick<Profile, 'username' | 'createdAt'> &
  Partial<Pick<Profile, 'displayName' | 'bio'>>
// Ada has no display name yet (like every account created before names existed).
const ADA_PROFILE: StoredProfile = { username: 'ada', displayName: null, bio: 'Math & engines', createdAt: CREATED_AT }
const NAMED_ADA_PROFILE: StoredProfile = { ...ADA_PROFILE, displayName: 'Ada Lovelace' }
...
interface ProfileStore {
  log: string[]
  get: (username: string) => StoredProfile | undefined
  me: () => StoredProfile
  save: (saved: Partial<MyProfile>) => void
}

function createProfileStore(profiles: Record<string, StoredProfile>): ProfileStore {
  ...
  return {
    ...
    // The signed-in user is always in the store.
...
function mockProfiles(profiles: Record<string, StoredProfile> = { ada: ADA_PROFILE }) {
...
function mockPatch(
  store: ProfileStore,
  respond: PatchResponder = (body, me) =>
    HttpResponse.json({
      id: 'u1',
  ...
  const requests: ProfilePatch[] = []
  server.use(
    http.patch<Record<string, never>, ProfilePatch>(apiUrl('/users/me'), async ({ request }) => {
...
async function replaceText(user: UserEvent, label: string, value: string) {
