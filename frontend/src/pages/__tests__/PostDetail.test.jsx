...
function mockPostApi({ post = makePost(), comments = [], pageSize = 20 } = {}) {
  const state = {
    post,
    comments,
    postStatus: 200,
  ...
  server.use(
    ...
    http.post(apiUrl('/posts/:id/comments'), async ({ request }) => {
...
describe('PostDetail', () => {
  it('shows the post under a "Post" header, with its comments oldest first', async () => {
    mockPostApi({
      comments: [
        ...
        makeComment('c2', { body: 'Second reply', author: { username: 'grace' } }),
      ],
    })
  ...
  describe('back button', () => {
    const backTo = async (user, pattern) => {
  ...
  describe('comments', () => {
    ...
    it("has no menu on other people's comments", async () => {
      mockPostApi({
        comments: [
          makeComment('c1', { body: 'From grace', author: { username: 'grace' } }),
