...
type SignInResponder = (
  body: unknown,
) => HttpResponse<DefaultBodyType> | Promise<HttpResponse<DefaultBodyType>>

function mockSignIn(respond: SignInResponder = () => HttpResponse.json(ADA)) {
...
async function fillAndSubmit(user: UserEvent, { email, password }: { email?: string; password?: string }) {
  if (email) {
