...
function mockFeed(path: string, pages: Post[][]) {
  const feed: { pages: Post[][]; requests: (string | null)[] } = { pages, requests: [] }
  server.use(
...
const created = (id: string, following: boolean) => stream().emit('post.created', { id, following })

describe('Home new-posts pill', () => {
  let uninstall: () => void
  let following: ReturnType<typeof mockFeed>
  let forYou: ReturnType<typeof mockFeed>
