...
// Profiles as served here: partial on purpose (e.g. no counts until a test adds them).
type ProfileFixtures = Record<string, Partial<Profile>>
...
function mockProfiles(profiles: ProfileFixtures = PROFILES, posts: Record<string, Post[][]> = {}) {
  const state: { status: number; postsStatus: number; requests: string[] } = {
    status: 200,
    postsStatus: 200,
    requests: [],
  }
  server.use(
    http.get<UsernameParams>(apiUrl('/users/:username/posts'), ({ params, request }) => {
    ...
    http.get<UsernameParams>(apiUrl('/users/:username'), ({ params }) => {
...
describe('Profile', () => {
  ...
  describe('follows', () => {
    const FOLLOW_PROFILES: ProfileFixtures = {
      ada: {
        ...PROFILES.ada,
        followingCount: 242,
        followerCount: 1200,
    ...
    it('closes the follow list when a row opens another profile', async () => {
      mockProfiles(FOLLOW_PROFILES)
      server.use(
        http.get<UsernameParams>(apiUrl('/users/:username/following'), ({ params }) =>
          HttpResponse.json({
            items:
    ...
    describe('freshness', () => {
      ...
      it('refetches the profile when it is shown again within 30s, keeping the cached counts on screen meanwhile', async () => {
        ...
        server.use(
          http.get<UsernameParams>(apiUrl('/users/:username'), async ({ params }) => {
            state.requests.push(params.username)
            await gate
