...
// may leave it empty.
const editProfileSchema = (hasDisplayName: boolean) =>
  z.object({
...
type EditProfileValues = z.output<ReturnType<typeof editProfileSchema>>;

// Loads the current bio and name (the `me` payload has no bio), then renders the form prefilled with it.
export function EditProfile() {
  // Only rendered under ProtectedRoute, so a signed-in user is always set here.
...
function EditProfileForm({ currentUsername, currentDisplayName, currentBio }: EditProfileFormProps) {
  ...
  const onSubmit = ({ displayName, username, bio }: EditProfileValues) => {
...
function removeQueryOnceUnobserved(queryClient: QueryClient, queryKey: QueryKey) {
...
function EditProfileShell({ username, children }: EditProfileShellProps) {
  return (
    <>
      <PageHeader
        title="Edit profile"
