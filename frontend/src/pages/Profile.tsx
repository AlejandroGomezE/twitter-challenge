...
import type { Profile as ProfileData } from '@/lib/api/types';
...
export function Profile() {
  // Always set: this page is only mounted on `/u/:username`.
  ...
  const isOwnProfile =
    !!user?.username && user.username.toLowerCase() === profile.username.toLowerCase();
...
interface FollowCountsProps {
  profile: ProfileData;
  openTab: FollowListTab | null;
  onOpenFollowList: (tab: FollowListTab) => void;
}

function FollowCounts({ profile, openTab, onOpenFollowList }: FollowCountsProps) {
  const { followingCount, followerCount } = profile;
  if (typeof followingCount !== 'number' || typeof followerCount !== 'number') return null;

  const items: { tab: FollowListTab; count: number; label: string }[] = [
...
function postCountLabel(postCount?: number) {
...
function ProfilePosts({ username, isOwnProfile }: ProfilePostsProps) {
