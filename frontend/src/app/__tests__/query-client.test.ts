...
function mockFeed(status: number) {
  const calls = { count: 0 }
  server.use(
...
const fetchFeed = (queryClient: QueryClient) =>
...
describe('createQueryClient', () => {
  ...
  it('lets callers override query defaults', () => {
    const { retry, staleTime } =
      createQueryClient({ queries: { retry: false } }).getDefaultOptions().queries ?? {}
