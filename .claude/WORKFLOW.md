...
| **Verify** | `/review-feature <slug>` | Run the app for real (`scripts/be-local` / `scripts/fe-local`) and check the acceptance criteria against the running app. Loop back to Build if something's wrong. |
| **Close** | `/close-feature <slug>` | Write/run the tests the change actually needs, open the PR, run a `review-pr` pass, merge with your explicit go-ahead. |
