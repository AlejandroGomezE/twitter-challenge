...
| **Close** | `/close-feature <slug>` | Write/run the tests the change actually needs (backend specs and frontend `*.test.jsx`, each in a `__tests__/` folder next to the code), open the PR, run a `review-pr` pass, merge with your explicit go-ahead, and stop any dev servers left running (`scripts/down-be` + `scripts/down-fe`). |
